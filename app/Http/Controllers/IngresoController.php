...
use compusystem\Http\Requests\IngresoFormRequest;
use compusystem\Ingreso;
use compusystem\DetalleIngreso;
use DB;
...
class IngresoController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request)
        {
           ...
           $ingresos=DB::table('ingreso as i')
            ...
            ->select('i.idingreso','i.fecha_hora', 'i.imagen','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado',DB::raw('sum(di.cantidad*precio_compra) as total'))
            ->where('i.num_comprobante','LIKE','%'.$query.'%')
            ->orderBy('i.idingreso','desc')
            ->groupBy('i.idingreso','i.fecha_hora', 'i.imagen','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado')
            ->paginate(10);
    ...
    public function create()
    {
     ...
     public function store (IngresoFormRequest $request) //store para almacenar
    {
    	try{
        	DB::beginTransaction();
        	$ingreso=new Ingreso;
            ...
            if (Input::hasFile('imagen')){
                ...
                $file->move(public_path().'/imagenes/ingreso/',$file->getClientOriginalName());
                $ingreso->imagen=$file->getClientOriginalName();
            }
